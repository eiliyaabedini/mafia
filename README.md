...
- Day one is introductions only: a single speaking round, no nomination and no elimination. The night that follows is the meeting night, where the Mafia simply learn each other, nobody acts and nobody dies. Ordinary days, with two rounds and a vote, begin on day two.
...
Android and iOS entry points remain compatible with the shared UI; native AI authentication is deferred. `shared/src/commonTest/.../GameRulesTest.kt` covers the deterministic rules only: the seven-seat composition, the introduction day and meeting night, the 40 percent defense and elimination thresholds, Godfather investigation immunity, and both Mafia roles counting toward the result. Run it with `./gradlew :shared:testAndroidHostTest`. It exercises no AI layer, makes no model request and spends no wallet credit, so a green run still does not verify wallet-funded gameplay; that remains a manual browser/play check.
