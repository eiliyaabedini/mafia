...
Run deployment only after packaging succeeds.

**Never let GitHub deploy this project.** The Vercel project is linked to the GitHub repository, and Vercel cannot build Kotlin/Wasm from source: a Git-triggered build produces an empty deployment that still becomes Production and serves 404 on every alias. On 2026-09-18 a push to `main` did exactly that, 27 seconds after a good prebuilt deploy. `vercel.json` sets `git.deploymentEnabled: false` so pushes no longer create deployments; keep it. If the site ever 404s after a push, restore the last prebuilt deployment with `vercel promote <deployment-url> --scope eiliya-s-projects --yes`, then check `vercel ls ai-mafia --scope eiliya-s-projects` for a deployment carrying the `ai-mafia-git-main` alias. The preparation script builds `:webApp:wasmJsBrowserDistribution`, then writes Vercel Build Output API v3 files to `.vercel/output`. If the production distribution is already current, use `python3 scripts/prepare-vercel.py --skip-build` before the same deploy command. The script preserves `.vercel/project.json`; on a fresh checkout, link the existing project with `vercel link --yes --project ai-mafia --scope eiliya-s-projects` first.
...
- Seven seats hold one Godfather, one Mafia, one Detective, one Doctor, and three Citizens. The Godfather and the Mafia are one team and know each other; a Detective who investigates the Godfather is told he is not Mafia, which is the role rather than a defect.
- Day one is introductions only: a single speaking round, no nomination and no elimination. The night that follows is the meeting night, where the Mafia learn each other, nobody acts and nobody dies. Ordinary days, with two rounds and a vote, begin on day two.
- Neither Mafia knows their partner during the introduction day; the private context and the human's own role card both withhold it until that first night. A Mafia may therefore press or target their own partner on day one and have to reverse it afterwards, which is exactly what the format is for.
- From day two each day shuffles the living players into a new speaking order, shared by that day's two discussion rounds. The human can occupy any position; the visible roster follows the order. Nomination voting follows discussion: each living person's portrait appears, and every other player answers yes/no in the day's order. A player can nominate several others or nobody. All votes become public in that order.
- Every player with more than 40 percent of the living table's nomination approvals enters defense, however many that is; there is no cap and no tie-break. If nobody clears that share, the day ends without defense or elimination. Each finalist gets one defense turn. Every living player then chooses one eligible finalist or abstains; the single highest non-abstaining total is eliminated only when it also exceeds 40 percent of the living table. A final tie, everyone abstaining, or a leading total under that share eliminates nobody. Self-votes are never legal.
...
- On your speaking turn you may type, or press `🎙 حرف بزن` to dictate. Recording runs entirely in the browser through `MediaRecorder` and is free; only the transcription is a paid AI Pass call, sent to `whisper-1` with `language: 'fa'`. Cancelling discards the audio locally and is never charged. By default the transcript is sent as your turn as soon as it arrives; the `بعد از ضبط، خودکار بفرست` switch beside the microphone turns that off, in which case the text lands in the field for you to edit first. The preference is saved locally with the other sound settings. A recording stops automatically after two minutes, leaving the turn ends the capture, and a transcription already in flight is allowed to finish so its charge still reaches the game total. Microphone access is requested only when you press the button.
...
- Each public AI line appears before it is spoken. While the voice is still being generated, that speaker's portrait carries a spinner and the panel names who the table is waiting for; once audio starts, the portrait shows moving bars instead. The same spinner marks whichever character the game is currently waiting on for a model response. Night and nomination never show it, because those phases must not reveal who is acting. The next character waits for playback to finish. Pause, mute, and leaving stop playback; a speech failure pauses further narration without changing the saved preference, while the text game continues.
...
- Background music has its own switch and volume in `صدا و موسیقی`. It defaults on and starts after the first real interaction, respecting browser playback rules. An explicit on/off choice is remembered locally. Six bundled tracks use a weighted random rotation without immediate repeats. Three instrumentals may open a session; the lower-priority versions only appear later. The vocal version is no longer left to a weighted draw: it is always the second track of a session, so anyone who stays past the opening song hears it. The rarer vocal cut keeps its very small weight and remains an occasional surprise. The one-minute version fades out over its final six seconds. Music has no wallet charge, softens during spoken dialogue, continues beneath the tutorial, and suspends when the game is paused or the tab is hidden. Browsers refuse to start audio before a real interaction, so playback begins on the first click, tap or key press rather than on page load. If that first attempt is blocked, the next genuine gesture clears the blocked state and retries; previously a single blocked start left the session silent until the player reopened the sound menu. [Soundtrack sources and playback rules](docs/MUSIC.md).
...
The game and AI Pass are both reachable from Iran without a VPN. On 2026-09-18 the game page, `aipass.one/aipass-sdk.js` and `aipass.one/v1/models` all answered in under 2.5 seconds from eight test servers in Tehran, Isfahan, Shiraz and Qom on eight different networks, and a player on ordinary MCI mobile internet signed in and played more than a hundred AI turns. AI Pass has no country, VPN or proxy blocking, and its fail2ban jail covers SSH only.

Proxy apps such as V2Ray route each domain by their own rules and resolve names themselves. A player whose proxy handles `vercel.app` correctly but mishandles `aipass.one` sees the page load while every AI Pass request stalls, and nothing reaches the AI Pass logs. The lobby's model check then times out after 40 seconds. For that case the lobby now asks the player to turn the proxy off and retry instead of showing the in-game timeout text.
...
Android and iOS entry points remain compatible with the shared UI; native AI authentication is deferred. `shared/src/commonTest/.../GameRulesTest.kt` covers the deterministic rules only: the seven-seat composition, the introduction day and meeting night, teammates staying hidden until that night, the 40 percent defense and elimination thresholds, Godfather investigation immunity, and both Mafia roles counting toward the result. Run it with `./gradlew :shared:testAndroidHostTest`. It exercises no AI layer, makes no model request and spends no wallet credit, so a green run still does not verify wallet-funded gameplay; that remains a manual browser/play check.
...
No speech request is retried automatically. Stopping an in-flight generation cannot guarantee that AI Pass has not already charged it. The SDK returns an audio Blob without per-request cost metadata, so the bridge measures that call through the wallet balance refresh the SDK performs after generation and stores it as a TTS usage entry. Audio buffers are held only for the current line and are not persisted or service-worker cached.
